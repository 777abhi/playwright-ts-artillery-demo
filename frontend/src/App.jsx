...
function App() {
  ...
  const [config, setConfig] = useState({ delay: 0, cpuLoad: 0 })
  const [result, setResult] = useState(null)
  const [loading, setLoading] = useState(false)
  const [savedMessage, setSavedMessage] = useState('')

  const handleSave = () => {
    setConfig({ delay, cpuLoad })
  ...
  const handleTrigger = async () => {
    ...
    setResult(null)
    try {
      const response = await fetch(`http://localhost:3001/process?delay=${config.delay}&cpuLoad=${config.cpuLoad}`)
